...
public class PhotoServiceImpl implements PhotoService {
	...
	public Photo dtoToEntity(PhotoDTO dto) {
		...
		System.out.println("1.@@@@Group is present: " + optGroup.isPresent());
		System.out.println("1.@@@@Group value: " + optGroup.orElse(null));
	...
	public Photo insertPhoto(PhotoDTO photoData) {
		Photo photo = dtoToEntity(photoData);
		 // groupNo의 유효성 검사
	    if (photo.getGroup() == null || photo.getGroup().getGroupNo() == null) {
	        // groupNo가 null인 경우에 대한 처리
	        // 예외를 던지거나 기본값을 설정해야 합니다.
	        throw new IllegalArgumentException("Invalid groupNo");
	    }
	...
	*/
	...
	public List<PhotoDTO> getPhotoListByGroupNo(Integer groupNo, Integer userNo) {
	    ...
	    List<PhotoDTO> photoDTOList = photoList.stream()
	            .map(entity -> {
	                PhotoDTO dto = entityToDto(entity);
	                return dto;
	            })
